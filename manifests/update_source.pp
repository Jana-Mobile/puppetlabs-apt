define apt::update_source(
  ...
  $sources_list_d_path  = $apt::params::sources_list_d
  $sources_list_d_split = split($sources_list_d_path, ',')
  $sources_list_d_dir   = $sources_list_d_split[-1]
  ...
  if $ensure == present {
    exec { "apt_update_${source_list}":
      command     => "${apt::params::provider} update -o Dir::Etc::sourcelist='${sources_list_d_dir}/${source_list}' -o Dir::Etc::sourceparts='-' -o APT::Get::List-Cleanup='0'",
